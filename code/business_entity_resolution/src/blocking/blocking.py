...
def build_candidates(
    s1, s2, s3, top_k=15, max_token_freq=35000, sample_size=None
):
  s1 = s1.copy()
  s2 = s2.copy()
  s3 = s3.copy()
  ...
  if sample_size and sample_size < len(s1):
    print(
        f"Sampling {sample_size} records from Source 1 for fast pipeline"
        " validation..."
    )
  ...
  for country, s1_country_df in s1.groupby("norm_country"):
    ...
    print(
        f"\nIndexing country '{country}': S1={total_s1:,},"
        f" Pool={len(s23_country_df):,}..."
    )

    # 1. Inverted Index (Name + Address)
    inverted_index = defaultdict(list)
    for eid, name, addr in zip(
        s23_country_df["entity_id"],
        s23_country_df["business_name"],
        s23_country_df["business_address"],
    ):
    ...
    inverted_index = {
        ...
        if len(eids) <= max_token_freq
    }

    # 3. Query S1 with live progress reporting
    start_time = time.time()
    for idx, (s1_id, name, addr) in enumerate(
        zip(
            s1_country_df["entity_id"],
            s1_country_df["business_name"],
            s1_country_df["business_address"],
        )
    ):
      if (idx + 1) % 10000 == 0 or (idx + 1) == total_s1:
        ...
        print(
            f"   Processed {idx + 1:,}/{total_s1:,} records ({rate:.1f}"
            " rec/s)...",
            flush=True,
        )
      ...
      if overlap_counts:
        ...
        if len(overlap_counts) <= top_k:
          top_matches = sorted(
              overlap_counts.keys(),
              key=overlap_counts.__getitem__,
              reverse=True,
          )
        else:
          top_matches = heapq.nlargest(
              top_k, overlap_counts.keys(), key=overlap_counts.__getitem__
          )

        for cand_id in top_matches:
          pair_rows.append(
              {"source1_entity_id": s1_id, "candidate_entity_id": cand_id}
          )

  print(
      f"\nTotal candidate pairs generated: {len(pair_rows):,} across"
      f" {len(s1):,} entities."
  )
